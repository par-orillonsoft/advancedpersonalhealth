...
require_once JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_cmigrator' . DS . 'library' . DS . 'parsers' . DS . 'com_k2' . DS . 'com_k2.php';

class CMigratorParserWordpress extends CMigratorParserK2 {
    ...
    public function parseContent($start, $limit) {
        ...
        $temps = WordPressHelper::parseContent($this->db, $this->articles, '#__k2_items');
    ...
    public function parseImages($start, $limit) {
        $ids = array();
        $this->setArticles($start, $limit);
        $temps = WordPressHelper::parseImages($this->db, $this->articles, '#__k2_items', $this);
        foreach($temps['ids'] as $temp) {
            $ids[] = $this->makeLink($temp);
        }
