 ...
 * @since		1.5
 */
class GigCalViewBand extends JView
{
	protected $state;
	protected $item;
	protected $form;
	...
	protected function addToolbar()
	{
		...
		{
			JToolBarHelper::apply('band.apply', 'JTOOLBAR_APPLY');
			JToolBarHelper::save('band.save', 'JTOOLBAR_SAVE');
		}
		if (!$checkedOut && $canCreate){			
			JToolBarHelper::custom('band.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
		}
		// If an existing item, can save to a copy.
		if (!$isNew && ($canCreate > 0)) {
			JToolBarHelper::custom('band.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
		}
		if (empty($this->item->id)) {
			JToolBarHelper::cancel('band.cancel', 'JTOOLBAR_CANCEL');
		}
		else {
			JToolBarHelper::cancel('band.cancel', 'JTOOLBAR_CLOSE');
		}

		JToolBarHelper::divider();
		JToolBarHelper::help('JHELP_COMPONENTS_GIGCAL_BAND_EDIT');
	}
}
